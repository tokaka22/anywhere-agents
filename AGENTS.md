...
<!-- agent:claude -->
- **Claude Code installation**: prefer the **native installer**. macOS: `curl -fsSL https://claude.ai/install.sh | sh`. Windows (PowerShell, no admin): `irm https://claude.ai/install.ps1 | iex`. Migration from npm/winget, auto-update behavior, and `~/.claude.json` healing → `references/claude-install.md`.
- **Claude Code effort level**: `/effort` exposes `low|medium|high|xhigh|max`. The persisted `effortLevel` key accepts `low|medium|high|xhigh`; `max` is session-only via the slash command. For persistent `max`, set env var `CLAUDE_CODE_EFFORT_LEVEL=max` in `~/.claude/settings.json`. Runtime precedence, override semantics, and the `--effort` launch flag → `references/claude-install.md` (Effort level section).
<!-- /agent:claude -->
